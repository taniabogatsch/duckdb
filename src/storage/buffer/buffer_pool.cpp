...
void EvictionQueue::PurgeIteration(const idx_t purge_size) {
	...
	for (idx_t i = 0; i < actually_dequeued; i++) {
		...
		if (debug_sleep_micros > 0) {
			...
			std::this_thread::sleep_for(std::chrono::microseconds(debug_sleep_micros));
		}
		if (handle) {
...
void BufferPool::PurgeQueue(const BlockHandle &block) {
	...
	auto &db_config = DBConfig::GetConfig(buffer_manager.GetDatabase());
	const auto queue_sleep_micros = db_config.options.debug_eviction_queue_sleep_micro_seconds;
