...
void StandardBufferManager::ReAllocate(shared_ptr<BlockHandle> &handle, idx_t block_size) {
	...
	auto handle_memory_usage = block_memory.GetMemoryUsage();
	D_ASSERT(block_memory.GetState() == BlockState::BLOCK_LOADED);
	D_ASSERT(handle_memory_usage == block_memory.GetBuffer(lock)->AllocSize());
	D_ASSERT(handle_memory_usage == block_memory.GetMemoryCharge(lock).size);

	auto req = block_memory.GetBuffer(lock)->CalculateMemory(block_size, handle->GetBlockHeaderSize());
	...
	if (memory_delta > 0) {
		...
		lock.unlock();
		auto reservation = EvictBlocksOrThrow(block_memory.GetMemoryTag(), NumericCast<idx_t>(memory_delta), nullptr,
		...
		block_memory.MergeMemoryReservation(lock, std::move(reservation));
	} else {
		// no need to evict blocks, but we do need to decrement 'current_memory'.
		block_memory.ResizeMemory(lock, req.alloc_size);
	}
	auto block_header_size = handle->GetBlockHeaderSize();
	block_memory.ResizeBuffer(lock, block_size, block_header_size, memory_delta);
}
