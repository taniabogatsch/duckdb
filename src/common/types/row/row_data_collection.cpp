...
RowDataCollection::RowDataCollection(BufferManager &buffer_manager, idx_t block_capacity, idx_t entry_size,
	...
	D_ASSERT(block_capacity * entry_size + entry_size > DEFAULT_BLOCK_SIZE);
}
...
RowDataBlock &RowDataCollection::CreateBlock() {
	blocks.push_back(
	    make_uniq<RowDataBlock>(MemoryTag::ORDER_BY, buffer_manager, block_capacity, entry_size, DEFAULT_BLOCK_SIZE));
	return *blocks.back();
}
...
void RowDataCollection::Merge(RowDataCollection &other) {
	...
	RowDataCollection temp(buffer_manager, DEFAULT_BLOCK_SIZE, 1);
	{
