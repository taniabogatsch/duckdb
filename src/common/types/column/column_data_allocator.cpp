...
BufferHandle ColumnDataAllocator::AllocateBlock(const idx_t size, const idx_t block_size) {
	D_ASSERT(type == ColumnDataAllocatorType::BUFFER_MANAGER_ALLOCATOR || type == ColumnDataAllocatorType::HYBRID);
	auto capacity = MaxValue<idx_t>(size, block_size);
	BlockMetaData data;
	data.size = 0;
	data.capacity = NumericCast<uint32_t>(capacity);
	auto pin = alloc.buffer_manager->Allocate(MemoryTag::COLUMN_DATA, capacity, false, &data.handle);
