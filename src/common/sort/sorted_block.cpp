...
void SortedData::CreateBlock() {
	auto capacity =
	    MaxValue((DEFAULT_BLOCK_SIZE + layout.GetRowWidth() - 1) / layout.GetRowWidth(), state.block_capacity);
	...
	if (!layout.AllConstant() && state.external) {
		heap_blocks.push_back(
		    make_uniq<RowDataBlock>(MemoryTag::ORDER_BY, buffer_manager, DEFAULT_BLOCK_SIZE, 1U, DEFAULT_BLOCK_SIZE));
...
void SortedBlock::CreateBlock() {
	auto capacity =
	    MaxValue((DEFAULT_BLOCK_SIZE + sort_layout.entry_size - 1) / sort_layout.entry_size, state.block_capacity);
	radix_sorting_data.push_back(make_uniq<RowDataBlock>(MemoryTag::ORDER_BY, buffer_manager, capacity,
	                                                     sort_layout.entry_size, DEFAULT_BLOCK_SIZE));
}
...
PayloadScanner::PayloadScanner(SortedData &sorted_data, GlobalSortState &global_sort_state, bool flush_p) {
	...
	rows = make_uniq<RowDataCollection>(global_sort_state.buffer_manager, DEFAULT_BLOCK_SIZE, 1U);
	rows->count = count;

	heap = make_uniq<RowDataCollection>(global_sort_state.buffer_manager, DEFAULT_BLOCK_SIZE, 1U);
