...
public:
	...
	fixed_size_vector(fixed_size_vector &&other) = delete;

	fixed_size_vector& operator=(const fixed_size_vector &other) = delete;
	fixed_size_vector& operator=(fixed_size_vector &&other) = delete;

public:
	void push_back(const T &ref) { emplace_at(len++, ref); }
	void push_back(T &&ref) { emplace_at(len++, std::move(ref)); }
	...
	const_iterator cbegin() { return ptr; }
	iterator end() { return ptr + len; }
	const_iterator cend() { return ptr + len; }
