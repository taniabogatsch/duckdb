...
struct RowDataBlock {
public:
	RowDataBlock(MemoryTag tag, BufferManager &buffer_manager, idx_t capacity, idx_t entry_size, idx_t block_size)
	    : capacity(capacity), entry_size(entry_size), count(0), byte_offset(0) {

		idx_t size = MaxValue<idx_t>(block_size, capacity * entry_size);
...
public:
	...
	static inline idx_t EntriesPerBlock(idx_t width, idx_t block_size) {
