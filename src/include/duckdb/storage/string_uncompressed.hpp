	...
	static idx_t StringAppendBase(BufferHandle &handle, ColumnSegment &segment, SegmentStatistics &stats,
		...
		for (idx_t i = 0; i < count; i++) {
...
#ifdef DEBUG
			GetDictionary(segment, handle).Verify(segment.block->block_manager.GetBlockSize());
#endif
			...
			if (DUCKDB_UNLIKELY(required_space >=
			                    StringUncompressed::GetStringBlockLimit(segment.block->block_manager.GetBlockSize()))) {
			...
			if (DUCKDB_UNLIKELY(use_overflow_block)) {
				...
				*dictionary_size += BIG_STRING_MARKER_SIZE;
				...
				D_ASSERT(NumericCast<idx_t>(*dictionary_size) <= segment.block->block_manager.GetBlockSize());
			...
			D_ASSERT(RemainingSpace(segment, handle) <= segment.block->block_manager.GetBlockSize());
#ifdef DEBUG
			GetDictionary(segment, handle).Verify(segment.block->block_manager.GetBlockSize());
#endif
		}
